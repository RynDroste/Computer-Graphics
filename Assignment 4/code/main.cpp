...
// Visualize Fresnel coefficients
bool visualizeFresnel = false;



bool isInShadow(glm::vec3 point, glm::vec3 light_position, Object *current_object){
	...
	for(int k = 0; k < objects.size(); k++){
		if(objects[k] == current_object){
			continue; 
		}
		
		Hit hit = objects[k]->intersect(shadow_ray);
		if(hit.hit && hit.distance > 0.0f && hit.distance < light_distance){
			return true;
		}
	}
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec3 view_direction, Material material, Object *current_object){

	glm::vec3 color(0.0);
	for(int light_num = 0; light_num < lights.size(); light_num++){
		if(isInShadow(point, lights[light_num]->position, current_object)){
			continue;
		}
...
// Fresnel coefficient
float fresnel(glm::vec3 incident, glm::vec3 normal, float n1, float n2){
	float cosTheta = glm::clamp(glm::dot(-incident, normal), -1.0f, 1.0f);
	float F0 = pow((n1 - n2) / (n1 + n2), 2.0f);
	return F0 + (1.0f - F0) * pow(1.0f - cosTheta, 5.0f);
}

glm::vec3 trace_ray(Ray ray, int depth = 0, float currentIOR = 1.0f){
	...
	if(closest_hit.hit){
		...
		// Check entering/exiting
		glm::vec3 normal = closest_hit.normal;
		bool entering = glm::dot(glm::normalize(ray.direction), normal) < 0.0f;
		if(!entering){
			normal = -normal;
		}
		
		glm::vec3 phong_color = PhongModel(closest_hit.intersection, closest_hit.normal, glm::normalize(-ray.direction), material, closest_hit.object);
		
		// Handle refraction
		if(material.isRefractive && depth < MAX_DEPTH){
			float n1 = entering ? currentIOR : material.refractiveIndex;
			float n2 = entering ? material.refractiveIndex : 1.0f;
			float eta = n1 / n2;
			
			glm::vec3 incident = glm::normalize(ray.direction);
			glm::vec3 refraction_direction = glm::refract(incident, normal, eta);
			float fresnelCoeff = fresnel(incident, normal, n1, n2);
			
			if(visualizeFresnel && depth == 0){
				// R=reflection, G=refraction
				color = glm::vec3(fresnelCoeff, 1.0f - fresnelCoeff, 0.0f);
			}
			else if(glm::length(refraction_direction) > 0.001f){
				float epsilon = 0.0001f;
				
				glm::vec3 reflection_direction = glm::reflect(incident, normal);
				Ray reflection_ray(closest_hit.intersection + epsilon * reflection_direction, reflection_direction);
				glm::vec3 reflection_color = trace_ray(reflection_ray, depth + 1, currentIOR);
				
				Ray refraction_ray(closest_hit.intersection + epsilon * refraction_direction, refraction_direction);
				float nextIOR = entering ? material.refractiveIndex : 1.0f;
				glm::vec3 refraction_color = trace_ray(refraction_ray, depth + 1, nextIOR);
				
				color = fresnelCoeff * reflection_color + (1.0f - fresnelCoeff) * refraction_color;
			} else {
				// Total internal reflection
				glm::vec3 reflection_direction = glm::reflect(incident, normal);
				float epsilon = 0.0001f;
				Ray reflection_ray(closest_hit.intersection + epsilon * reflection_direction, reflection_direction);
				color = trace_ray(reflection_ray, depth + 1, currentIOR);
			}
		}
		// Handle reflection
		else if(material.reflectivity > 0.0 && depth < MAX_DEPTH){
			glm::vec3 reflection_direction = glm::reflect(glm::normalize(ray.direction), closest_hit.normal);
			float epsilon = 0.0001f;
			Ray reflection_ray(closest_hit.intersection + epsilon * reflection_direction, reflection_direction);
			glm::vec3 reflection_color = trace_ray(reflection_ray, depth + 1, currentIOR);
			color = (1.0f - material.reflectivity) * phong_color + material.reflectivity * reflection_color;
		} else {
			color = phong_color;
		}
	}else{
...
void sceneDefinition (){
	...
	blue_specular.specular = glm::vec3(0.6);
	blue_specular.shininess = 100.0;
	blue_specular.reflectivity = 0.8f; // Make the blue sphere reflective
